...
<title>Test mobile — Mission d'intégration</title>
<style>
  :root{--muted:#888;--border:#ddd;--panel:#fff;}
  html,body{height:100%;}
  body{font-family:system-ui,-apple-system,sans-serif;background:#e9e7e1;margin:0;
    display:flex;align-items:center;justify-content:center;padding:32px 16px;}
  .hidden{display:none;}
  .phone{
    width:375px;height:780px;background:#111;border-radius:52px;
    padding:14px;box-shadow:0 30px 60px rgba(0,0,0,.25), 0 0 0 2px #000;
    position:relative;flex-shrink:0;
  }
  .phone::before{
    content:"";position:absolute;top:14px;left:50%;transform:translateX(-50%);
    width:120px;height:28px;background:#111;border-radius:0 0 18px 18px;z-index:5;
  }
  .screen{
    width:100%;height:100%;background:#faf9f6;border-radius:38px;overflow:hidden;
    position:relative;display:flex;flex-direction:column;
  }
  .statusbar{
    height:44px;flex-shrink:0;display:flex;align-items:center;justify-content:space-between;
    padding:0 24px;font-size:13px;font-weight:600;color:#111;
  }
  .screen-content{flex:1;overflow-y:auto;padding:8px 16px 24px;-webkit-overflow-scrolling:touch;}
  .welcome{max-width:100%;margin:40px auto 0;text-align:center;color:#555;padding:0 4px;}
  .welcome h2{font-size:18px;margin-bottom:8px;}
  .welcome p{font-size:14px;line-height:1.5;}
  .welcome button{margin-top:16px;padding:12px 20px;border-radius:10px;border:none;
    background:#2f6f4f;color:#fff;font-size:14px;font-weight:600;cursor:pointer;width:100%;}
  @media (max-width: 480px){
    body{padding:0;background:#faf9f6;display:block;}
    .phone{
      width:100vw;height:100vh;height:100dvh;border-radius:0;padding:0;
      box-shadow:none;background:#faf9f6;
    }
    .phone::before{display:none;}
    .screen{border-radius:0;}
    .statusbar{padding:12px 20px 4px;height:auto;}
  }
</style>
</head>
<body>
  ...
  <div class="screen">
    ...
    <div class="screen-content">
      <section class="welcome" id="welcomeSection">
        <h2>👋 Bienvenue</h2>
        <p>Voici un aperçu de la nouvelle mission d'intégration, en conditions mobiles.</p>
        <button type="button" onclick="startTreasureHunt()">
          💬 Un message de Sam
        </button>
      </section>
...
updateMissionClock();
setInterval(updateMissionClock, 15000);
</script>
<script>
/* =========================================================
